...
or OpenAI involved. decode_to_wav / split_wav are monkeypatched (the real
...
from app.clients.openai_transcriber import DiarizedTranscription
    ...
    async def transcribe(self, wav_path, language=None):
        ...
        if wav_path == "chunk_0000.wav":
            return DiarizedTranscription(
                language="it",
                segments=[
                    {"start": 1.0, "end": 2.0, "speaker": "Speaker 1", "text": "uno"}
                ],
            )
        return DiarizedTranscription(
            language=None,  # hint from chunk 0 is reused by the worker
            segments=[
                {"start": 0.5, "end": 1.5, "speaker": "Speaker 2", "text": "due"}
            ],
        )
...
async def test_process_job_chunks_and_publishes(fakes):
    ...
    assert final_transcript["model"] == "gpt-4o-transcribe-diarize"
    ...
    assert diarization["segments"][1] == {
        "start": 5.5, "end": 6.5, "speaker": "Speaker 2", "text": "due", "chunk": 1,
    }
    ...
    assert progress[0].payload["segments"] == [
        {"start": 1.0, "end": 2.0, "speaker": "Speaker 1", "text": "uno", "chunk": 0},
    ]
