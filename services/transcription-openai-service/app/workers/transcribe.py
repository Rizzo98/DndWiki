"""OpenAI Speech-to-Text transcription worker (chunked uploads).
...
of the pipeline (speaker-service, content-service, web UI) is unaware of the
backend:
...
Because OpenAI caps each upload at 25 MB, long recordings are decoded
locally (ffmpeg -> 16 kHz mono WAV) and sliced into fixed-length chunks
(OPENAI_CHUNK_SECONDS); each chunk is sent to the API separately, its
diarized segments are offset back into the session timeline and merged in
sequence. After every chunk the partial artifacts are overwritten in MinIO
(same URIs the UI polls) and a transcription.progress event is published,
so results become visible before the session completes — exactly like the
on-prem worker. Speaker identification then runs locally, unchanged.
...
from app.clients.openai_transcriber import OpenAITranscriber
...
_transcriber: OpenAITranscriber | None = None
...
def get_transcriber(settings: ServiceSettings) -> OpenAITranscriber:
    global _transcriber
    if _transcriber is None:
        _transcriber = OpenAITranscriber(settings)
    return _transcriber
...
async def process_job(
    ...
    logger.info("Transcribing session %s (%s) via OpenAI", session_id, audio_uri)
    ...
    try:
        ...
        # fixed-length chunks, each well below OpenAI's 25 MB upload cap.
        ...
        max_bytes = settings.openai_max_upload_mb * 1024 * 1024
        chunks = await asyncio.to_thread(
            split_wav, wav_path, settings.openai_chunk_seconds, max_bytes, chunk_dir
        )
        if not chunks:
        ...
        language = settings.openai_transcription_language.strip() or None
...
async def _store_artifacts(
    ...
    transcript = build_transcript(
        session_id, language, settings.openai_transcription_model, segments
    )
