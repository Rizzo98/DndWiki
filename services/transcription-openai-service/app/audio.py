"""Local audio decode + chunking for the OpenAI transcription worker.

OpenAI's speech-to-text API caps each upload at 25 MB, so a long session
recording cannot be sent whole. Mirroring the on-prem WhisperX worker, the
recording is decoded locally and sliced into fixed-length chunks; each chunk
WAV is uploaded separately and the responses are spliced back into the
session timeline by the worker.
...
from app.clients.openai_transcriber import TranscriptionError
...
def split_wav(
    ...
    instead of uploading an oversized file that OpenAI rejects.
    """
    ...
    if bytes_per_chunk > max_bytes:
        raise TranscriptionError(
            f"OPENAI_CHUNK_SECONDS={chunk_seconds:g} produces ~"
            f"{bytes_per_chunk / (1024 * 1024):.1f} MiB WAV chunks, above "
            f"OPENAI_MAX_UPLOAD_MB={max_bytes / (1024 * 1024):.0f} "
            f"(OpenAI caps each transcription upload at 25 MB). "
            f"Lower OPENAI_CHUNK_SECONDS."
        )
