"""MinIO (S3-compatible) object storage for raw audio and transcripts.

Uses aioboto3 so uploads stream chunk-by-chunk (no full-file buffering) and
all URLs handed to clients are short-lived presigned ones (never public).
"""
...
class ObjectStorage:
    ...
    async def delete_object(self, bucket: str, key: str) -> None:
        """Remove "bucket/key" (used when a recording is replaced and its
        transcript/diarization artifacts are superseded)."""
        async with self._session.client("s3", **self._client_kwargs()) as s3:
    ...
    async def presigned_put(self, bucket: str, key: str, expires_sec: int = 900) -> str:
        """Short-lived PUT URL (for future direct-to-MinIO uploads from the app)."""
        async with self._session.client("s3", **self._client_kwargs()) as s3:
            return await s3.generate_presigned_url(
                "put_object",
                Params={"Bucket": bucket, "Key": key},
