"""Local audio decode + chunking for the Deepgram transcription worker.

Deepgram's pre-recorded endpoint accepts the audio as the request body; very
long recordings risk per-request latency spikes and run into plan-dependent
payload limits. Mirroring the on-prem WhisperX worker and the OpenAI variant,
the recording is decoded locally and sliced into fixed-length chunks; each
chunk WAV is uploaded separately and the responses are spliced back into the
...
def split_wav(
    ...
    instead of uploading an oversized file that Deepgram rejects.
    """
    ...
    if bytes_per_chunk > max_bytes:
        raise TranscriptionError(
            f"DEEPGRAM_CHUNK_SECONDS={chunk_seconds:g} produces ~"
            f"{bytes_per_chunk / (1024 * 1024):.1f} MiB WAV chunks, above "
            f"DEEPGRAM_MAX_UPLOAD_MB={max_bytes / (1024 * 1024):.0f}. "
            f"Lower DEEPGRAM_CHUNK_SECONDS."
        )
