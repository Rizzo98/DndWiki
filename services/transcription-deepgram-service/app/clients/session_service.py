...
Identical to the on-prem WhisperX worker's client: the transcription worker
never touches dnd_sessions tables directly; it moves the session through the
pipeline state machine (recorded -> transcribing -> transcribed, or ->
failed) and attaches transcript artifacts via the internal endpoints,
