"""MinIO (S3-compatible) object storage for character portraits.

Same pattern as user-service/session-service: uploads stream through
aioboto3 and every URL handed to clients is a short-lived presigned one
(never public). Images live under the shared 'wiki-assets' bucket
('characters/{page_id}.{ext}').
"""
...
#: Multipart content type -> file extension for character portraits.
...
class ObjectStorage:
    """Thin wrapper over the S3 client for the wiki-assets bucket."""
